...
### Migración
- [x] Generar migración `add_behavior_type_to_messages`

### Schema y Contexto Clínico
- [x] Añadir `behavior_type` al schema de `Alethea.Clinical.Message`
- [x] Añadir `Alethea.Clinical.list_recent_messages(patient_id, limit)`
- [x] Crear `Alethea.Clinical` context module

### Seguridad del Webhook
- [x] Actualizar `WhatsappWebhookController.receive/2` para validar `X-Hub-Signature-256`
- [x] Configurar `WHATSAPP_APP_SECRET`

### Configuración del LLM
- [x] Añadir System Prompt socrático a `config/config.exs`
- [x] Configurar variables de entorno para LLM en `config/runtime.exs`

### AI Chain — Contexto de Conversación
- [x] Actualizar `GuidedConversationChain.run/1` para usar `patient_context`
- [x] Mantener `behavior_type: :elicited` en el resultado

### Worker Oban — Pipeline Clínico Completo
- [x] Actualizar `AletheaJobs.ProcessMessageWorker.perform/1` para ejecutar el pipeline clínico completo.

### Plug — Raw Body Preservation
- [x] Configurar `CacheBodyReader` en el endpoint.

### Tests
- [x] Test del pipeline completo en `test/alethea_jobs/process_message_worker_test.exs`
- [x] Test de firmas en `test/alethea_web/controllers/whatsapp_webhook_controller_test.exs`
