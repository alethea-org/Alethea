ExUnit.start()
Ecto.Adapters.SQL.Sandbox.mode(Alethea.Repo, :manual)

Mox.defmock(Alethea.WhatsApp.ClientMock, for: Alethea.WhatsApp.ClientBehaviour)
Mox.defmock(Alethea.AI.RoBERTaWorkerMock, for: Alethea.AI.RoBERTaWorkerBehavior)
Mox.defmock(Alethea.AI.SessionSummaryChainMock, for: Alethea.AI.SessionSummaryChainBehavior)
Mox.defmock(Alethea.AI.WeeklySummaryChainMock, for: Alethea.AI.WeeklySummaryChainBehavior)
Mox.defmock(Alethea.AI.PhiWorkerMock, for: Alethea.AI.PhiWorkerBehaviour)
