...
- [x] 5.1 RED: `test/alethea/ai/chains/clinical_hypothesis_chain_test.exs` — `ClinicalHypothesisChain.supported_providers() == [:local]` (scenario "Chain is local-only").
- [x] 5.2 RED: `build_prompt/1` test — given `%{question:, excerpts:}`, assert numbered excerpts + question present, and `refute prompt =~ "chunk_id"` / `"resource_id"` / `"target_behavior_id"` (scenario "Prompt carries no citation identifiers").
- [x] 5.3 RED: `parse/1` table-driven test — `for bad <- [malformed_json, missing_key_json, "", "   "], do: assert parse(bad) == {:error, :unparseable}` (scenario "Malformed output fails loud").
- [x] 5.4 RED: `parse/1` happy path — valid `%{"hypothesis" => "..."}` JSON → `{:ok, %{hypothesis: trimmed}}`.
- [x] 5.5 RED: prompt golden test — `suggested_system_prompt/0` contains verbatim `"NO es un diagnóstico ni una recomendación terapéutica"`, verbatim `"no indiques tratamiento, medicación ni derivación"`, and the server-owns-disclaimer bullet text.
- [x] 5.6 GREEN: create `lib/alethea/ai/chains/clinical_hypothesis_chain.ex` — mirror `clinical_consultation_chain.ex` shape exactly: `@behaviour ChainBehaviour`, `run/1`, `run!/1`, `suggested_system_prompt/0` (exact design system prompt text), `suggested_max_tokens/0` → `384`, `supported_providers/0` → `[:local]`, `@doc false hypothesis_schema/0`, `build_prompt/1`, `parse/1`, private `do_run/2` with `:telemetry.execute` tagged `chain: :clinical_hypothesis`.
...
- [x] 6.1 RED: `LLMConfig.get_and_build(:consultation_hypothesis)` resolves `{:ok, _, %OllamaChat{}}` with `provider == :local` (scenario "New chain resolves correctly").
- [x] 6.2 RED: regression test — all six prior `chain_name` values (`:guided_conversation`, `:session_summary`, `:weekly_summary`, `:weekly_report`, `:pattern_proposal`, `:consultation_synthesis`) still resolve identically via `get_and_build/1` (scenario "Existing chain clauses are untouched").
- [x] 6.3 GREEN: modify `lib/alethea/ai/llm_config.ex` — add `| :consultation_hypothesis` to `@type chain_name`, add `defp chain_module(:consultation_hypothesis), do: Alethea.AI.Chains.ClinicalHypothesisChain` clause. No other line touched.
- [x] 6.4 Modify `config/test.exs` — add `config :alethea, :clinical_hypothesis_chain, Alethea.AI.ClinicalHypothesisChainMock` (mirrors existing `:clinical_consultation_chain` line).
- [x] 6.5 Modify `test/test_helper.exs` — add `Mox.defmock(Alethea.AI.ClinicalHypothesisChainMock, for: Alethea.AI.Chains.ChainBehaviour)`.
- [x] 6.6 RED/GREEN: Mox integration test — `import Mox`, `setup :verify_on_exit!`, mock `run/1` returns text derived only from given excerpts; assert dispatch reaches the mock in `:test` env.

## Phase 7: #229b — boundary regression (Requirement: No Wiring Into the Real Consultation Flow)

- [x] 7.1 Diff `lib/alethea/ai/chains/clinical_consultation_chain.ex` against pre-change version — assert byte-identical (scenario "ClinicalConsultationChain is byte-unchanged"); run its existing prompt regression test unmodified. (`git diff --stat` shows zero diff on the file and its test; both ran green as part of the `test/alethea/ai/chains/` batch.)
- [x] 7.2 STATIC: source scan of `Consultation.Live`/`ConsultationLive` call sites — `refute` any reference to `Hypothesis`, `HypothesisPolicy`, or `ClinicalHypothesisChain` (scenario "No call site touches the real consultation flow"). (Manually verified via `rg` against `consultation_live.ex`, `followup_state.ex`, `router.ex` — zero matches; not added as a permanent automated test, since #229 explicitly does not touch these files.)
- [ ] 7.3 Run full suite `mix test` — confirm zero regressions across #226a/#226b tests. **NOT RUN** — apply-phase protocol explicitly forbids running the full suite (prior stalls); deferred to orchestrator/sdd-verify.

## Phase 8: Final verification

- [ ] 8.1 `mix precommit` (compile, format, test) on both slices before opening each PR. **NOT RUN** — includes full `mix test`, deferred to orchestrator per protocol. `mix compile --warnings-as-errors --force` and `mix format --check-formatted` (the non-full-suite parts) were run directly and are clean for all #229b files.
- [ ] 8.2 Confirm PR 1 diff excludes any `clinical_hypothesis_chain.ex`/`llm_config.ex` content (clean slice boundary); confirm PR 2 diff, based on PR 1's branch, shows only PR 2's own files when compared against PR 1 (not against the tracker) — retarget/rebase if GitHub shows PR 1's changes inside PR 2. **Deferred** — PR admin task for the orchestrator, not applicable during apply (no commit/PR made in this batch).
